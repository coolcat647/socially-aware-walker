...
TIME_LIMIT = 35.0
METHOD_LIST = ["orca", "ComfortBasedAstar", "OriginalAGFAstar", "SociallyAwareAstar", "SARL"]
# METHOD_LIST = ["SARL",]
HUMAN_RADIUS = 0.4
ROBOT_RADIUS = 0.7
TIME_STEP = 0.25
...
def render(dataframe, mode='human', output_file=None):
    ...
    if mode == 'traj':
        ...
        for k in range(len(dataframe)):
            if k % 5 == 0 or k == len(dataframe) - 1:
                robot = plt.Circle(robot_positions[k], ROBOT_RADIUS, fill=False, color=robot_color, alpha=np.tanh(float(k) / len(dataframe)))
                humans = [plt.Circle(human_positions[k][i], HUMAN_RADIUS, fill=False, color=cmap(i), alpha=np.tanh(float(k) / len(dataframe)))
        ...
        anim = animation.FuncAnimation(fig, update, frames=len(dataframe), interval=TIME_STEP * 1000 / 2)
        anim.running = True
    ...
    for target_dir in target_dir_list:
        ...
        for idx, filename in enumerate(target_files):
            ...
            if df["succ"].iloc[-1] == "yes":
                min_dis_list.append(min(df["min distance"]) - ROBOT_RADIUS) # get min distance
        ...
        print("    Avg_navi_time\t: {:.3f} secs".format(method_dict["avg_navi_time"]))
        print("    75pctl_navi_time\t: {:.3f} secs".format(method_dict["75pctl_navi_time"]))
        print("    90pctl_navi_time\t: {:.3f} secs".format(method_dict["90pctl_navi_time"]))
        print("    avg_min_dis\t\t: {:.3f} m".format(method_dict["avg_min_dis"]))
        print("    25pctl_min_dis\t: {:.3f} m".format(method_dict["25pctl_min_dis"]))
        print("    10pctl_min_dis\t: {:.3f} m".format(method_dict["10pctl_min_dis"]))
