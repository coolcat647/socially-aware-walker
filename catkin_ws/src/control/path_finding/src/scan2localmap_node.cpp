...
void Scan2LocalmapNode::trk3d_cb(const walker_msgs::Trk3DArray::ConstPtr &msg_ptr) {
    ...
    for(int i = 0; i < msg_ptr->trks_list.size(); i++) {
        ...
        if(map_x < map_width && map_y < map_height) {
            switch(agf_type_){
                case 0:
                    localmap_utils::apply_original_agf(localmap_ptr_, idx, yaw, std::ceil(speed * 1.2), 100);
                    break;
                case 1:
                    localmap_utils::apply_social_agf(localmap_ptr_, idx, yaw, std::ceil(speed * 1.2), 100, true);
                    break;
                case 2:
                    localmap_utils::apply_social_agf(localmap_ptr_, idx, yaw, std::ceil(speed * 1.2), 100, false);
                    break;
            }
        }
    }
